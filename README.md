...
> Коротко для жюри: RegRadar показывает путь от официального документа до понятной карточки риска, списка затронутых клиентов, evidence-фрагментов, уведомлений и RAG-чата по источнику. Интерфейс — это админ-панель комплаенс-команды, а не личный кабинет одного сотрудника.
